...
ChromaDB and sentence-transformers are heavy optional dependencies. If they are
not installed (or fail to initialise), Memory transparently runs in a disabled
no-op mode: stores become no-ops and retrievals return empty/neutral results.
Every caller already treats memory as best-effort, so the agent keeps running.
"""
...
class Memory:
    def __init__(self, path: str = settings.CHROMA_PATH):
        ...
        try:
            ...
            logger.warning(
                "ChromaDB memory unavailable (%s). Running with memory disabled.", e
            )
